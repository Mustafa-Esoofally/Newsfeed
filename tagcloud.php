...
<body>
    ...
    <div id="container" style="width: 1350px; height: 550px;"></div>
...
<script>
    ...
    <?php
    $query = "SELECT Id ,SUBSTRING_INDEX(SUBSTRING_INDEX(newsfeed.title, ' ', numbers.n), ' ', -1) Title FROM (SELECT 1 n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4) numbers INNER JOIN newsfeed ON CHAR_LENGTH(newsfeed.Description) -CHAR_LENGTH(REPLACE(newsfeed.Description, ' ', ''))>=numbers.n-1 order by Id desc limit 100 ";

    $result = mysqli_query($db, $query);
    $words1 = mysqli_fetch_all($result); //words is  array to be inserted 
    $remove = array("no", "and", "startups", "a", "the", "for", "only", "start-up", "to", "this", "reasons", "away", "in", "startup", "you", "new", "your", "why ", "cuts", "can", "what", "out", "on", "and", "at", "in", "they", "its", "is", "an", "from", "will", "uses", "with", "by", "who", "on", "By", "news", "how", "of", "ok", "in",  "that", "are", "Its", "tech", "2020", "us", "company", "alert", "more", "up", "as", "you", "[funding", "from","be");
    ?>
    ...
    chart.listen("pointClick", function(e) {
        ...
        var url1 = "tagcloudword.php";
        window.open(url1);
        // return fired_button;
        document.cookie = "word=" + fired_button;
        "path=tagcloudword.php";
    });
    chart.draw();
</script>