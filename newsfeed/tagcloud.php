...
<script>
    ...
    $Query = "SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(newsfeed.title, ' ', numbers.n), ' ', -1) Title FROM (SELECT 1 n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4) numbers INNER JOIN newsfeed ON CHAR_LENGTH(newsfeed.Description) -CHAR_LENGTH(REPLACE(newsfeed.Description, ' ', ''))>=numbers.n-1 order by Id desc limit 200 ";
    $Fullwordarray = array();
